...
description: 贯穿项目全生命周期的 LLM 友好文档体系构建能力。定位为跨阶段支撑 Skill，而非单一阶段。新项目模式在架构设计后、开发前介入（生成 CLAUDE.md 开发指南 + 模块开发文档）；文档模式在开发后/迭代中随时使用（梳理代码、更新文档、同步变更）。当用户需要为项目创建AI编程文档、建立项目索引、梳理代码、新建项目规划、或提到llm.txt/CLAUDE.md时使用。
metadata:
  author: zhairuitao
  version: "1.1"
  language: zh-CN
...
本 Skill 是**贯穿项目全生命周期的支撑能力**，而非单一阶段的工具。它与主流程（/product → /architect → /dev-pipeline → /retrospect）的衔接关系如下：

```
/product → /architect → ┌─ /build_doc_sys（新项目模式）─┐ → /dev-pipeline → /retrospect
  产品定义    架构设计    架构后、开发前介入                开发全流程         复盘优化
                         生成 CLAUDE.md 开发指南
                         + 模块开发文档
                                                          │
                                                          └→ /build_doc_sys（文档模式）
...
| 时机 | 模式 | 目的 | 产出 |
|------|------|------|------|
| 架构设计完成后、开发前 | 新项目模式 | 为开发阶段准备 LLM 友好的上下文 | CLAUDE.md 开发指南 + 模块开发文档 |
| 开发过程中（按需） | 文档模式（增量） | 新增模块的文档补充 | 新模块的 README + api + data-model |
| 版本交付后 | 文档模式（全量/增量） | 同步代码变更到文档体系 | 更新 CLAUDE.md + 各模块文档 + CHANGELOG |
| 复盘优化后 | 文档模式（增量） | 落实复盘中的约束变更到文档 | 更新受影响的文档 |

### 与其他 Skill 的协作

| Skill | 关系 | 说明 |
|-------|------|------|
| /product | 上游 | 产品定义影响文档的模块划分和功能描述 |
| /architect | 上游 | 架构设计决定文档的目录结构和分层方式 |
| /dev-pipeline | 并行/下游 | 开发产出是文档梳理的输入；文档体系为开发提供上下文 |
| /retrospect | 上游 | 复盘中的约束变更需要同步到文档体系 |
