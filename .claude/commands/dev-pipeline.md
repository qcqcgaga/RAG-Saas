...
6. **生成缺陷定位修复记录**（每次人工测试发现问题的修复过程必须独立记录）：
   - 每轮人工测试的问题发现/定位/修复过程生成独立文档
   - 文件名格式：`{project}-manual-test-fix-log-r{N}.md`（N为修复轮次，从1开始）
   - 每个缺陷记录包含：现象、根因分析、修复方案、修改文件、验证结果
   - 输出到 `docs/pipeline/{run_id}/` 目录
   - **同步更新 `docs/pipeline/index.md`** 关联该文档
   - **测试引导文档（manual-test-guide.md）由用户填写测试结果，AI不得修改此文件**
...
│       │   ├── docchat-manual-test-fix-log-r1.md
